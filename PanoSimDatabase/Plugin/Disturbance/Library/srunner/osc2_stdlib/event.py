...
class Event:
    ...
    def drive_distance(cls, other_car, distance):
        ...
        actor = PanoSimDataProvider.get_actor_by_name(cls.other_car)
        ret = DriveDistance(actor, cls.distance)
        return ret

    @classmethod
    def abs_distance_between_locations(cls, reference_actor, actor):
        actor1 = PanoSimDataProvider.get_actor_by_name(reference_actor)
        actor2 = PanoSimDataProvider.get_actor_by_name(actor)
    ...
    def near_collision(cls, other_car, distance):
        ...
        actor = PanoSimDataProvider.get_actor_by_name(cls.other_car)
        reference_actor = PanoSimDataProvider.get_actor_by_name("ego_vehicle")
